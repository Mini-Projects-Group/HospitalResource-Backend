...
const verifySellerApprovingOrder = async (req, res, next) => {
    ...
    let result = await query(`SELECT * FROM orders WHERE order_id=${order_id} AND seller_id=${user_id};`);
    // NOT FOUND
    if( result.length == 0 ) {
        res.status(404);
        ...
        res.status(404);
        return res.json({error:true, message: "Order Has Already Been Delivered."})
    } else {
        next();
    }
