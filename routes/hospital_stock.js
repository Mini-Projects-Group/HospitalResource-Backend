...
router.get(
    ...
    async(req, res) => {
      ...
      hospital_items_list
        ...
        let hospital = await query(`SELECT * FROM hospital NATURAL JOIN credential WHERE hospital_id=${current_order[0].hospital_id};`)

        await sendMail('hospitalresourcemanagement@gmail.com', hospital[0], 'Order Confirmation', current_order[0], req.user)
        return res.json({
            error: false,
            message: "Approved",
        });
